...
with tab5:
    ...
    for idx, row in headstart_df.iterrows():
        card_class = "position-1" if row["Headstart (%)"] >= 7 else "position-2" if row["Headstart (%)"] >= 4 else ""
        st.markdown(f'''
    ...
    # Visualization: Headstart Distribution
    st.markdown("---")
    st.markdown("#### 📊 Headstart Distribution")
    if headstart_data:
        headstart_chart_data = pd.DataFrame([
            {
                "Driver": f"{d['Driver']} ({d['Team']})",
                "Headstart": d["Headstart (%)"],
                "Color": driver_colors[d["Driver"]]
            }
            for d in headstart_data
        ])
        fig = px.bar(
            headstart_chart_data,
            x="Driver",
            y="Headstart",
            color="Driver",
            text="Headstart",
            title="Driver Headstart Distribution",
            color_discrete_map={row["Driver"]: row["Color"] for _, row in headstart_chart_data.iterrows()}
        )
        fig.update_traces(textposition="outside", texttemplate="%{text}%")
        fig.update_layout(
            height=500,
            xaxis_title="Driver",
            yaxis_title="Headstart (%)",
            showlegend=False,
